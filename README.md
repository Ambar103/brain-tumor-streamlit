...
## Model
An EfficientNetB0 backbone was fine-tuned on the [Kaggle Brain Tumor MRI Dataset](https://www.kaggle.com/datasets/masoudnickparvar/brain-tumor-mri-dataset) (glioma, meningioma, pituitary, no tumor — ~7,000 MRI images). Training used transfer learning in two phases (frozen head, then fine-tuning the top layers) with data augmentation. Test-set accuracy: **77.5%**. The trained model is hosted on [Hugging Face Hub](https://huggingface.co/Ambar10/brain-tumor-efficientnetb0) and downloaded automatically by the app. See `training/train.py` for the full pipeline and `training/artifacts/` for the confusion matrix and classification report.
